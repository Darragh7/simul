...
const FriendsList = ({ onShowCreateGroup = null }) => {
    const { user } = useAuth();
    ...
    const handleCreateGroup = () => {
        if (onShowCreateGroup && friends.length > 0) {
            onShowCreateGroup(friends);
        } else if (friends.length === 0) {
            setError("You need to have friends to create a group");
        }
    };
