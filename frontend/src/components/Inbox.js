...
const Inbox = () => {
    ...
    const handleJoinGroup = async (groupId, groupName) => {
        try {
            ...
            // 1. Update the groups document to add the user to members and remove from pendingMembers
            ...
            // 2. Add the user to the members subcollection in the group
            ...
            // 3. Update user_groups collection to include this group for the user
            ...
            // 4. Update any invitation notifications in the inbox
            ...
            inboxSnapshot.forEach(async (document) => {
                await updateDoc(doc(db, "inbox", document.id), {
                    status: "accepted"
                });
            });

            setGroupInvites(groupInvites.filter((invite) => invite.id !== groupId));
            setSuccess(`You have joined the group "${groupName || 'Unknown group'}"`);
        } catch (err) {
    ...
    const handleRejectGroupInvite = async (groupId) => {
        try {
            ...
            // Update any invitation notifications in the inbox
            const inboxQuery = query(
                collection(db, "inbox"),
                where("to", "==", user.email),
                where("groupId", "==", groupId),
                        ...
                        {groupInvites.map((invite) => (
                            <li key={invite.id} className="invite-item">
                                ...
                                <div className="invite-actions">
                                    <button onClick={() => handleJoinGroup(invite.id, invite.name)}>Join</button>
